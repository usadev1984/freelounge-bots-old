...
def register_tasks(sched):
    ...
    sched.register(task, hours=192) # (1/4) * cache duration CACHE DURATION (needs checking???)
...
def send_to_single_inner_photo(chat_id, ev, reply_to=None, force_caption=None, photo=None):
    if isinstance(ev, rp.Reply):
        ...
        kwargs2["photo"] = photo
        return bot.send_photo(chat_id, rp.formatForTelegram(ev), parse_mode="HTML", **kwargs2)
    ...
    kwargs2 = {}
    kwargs2["photo"] = telebot.types.InputFile(photo)
    if user:
        chat_id = user.id
    return bot.send_photo(chat_id, **kwargs2)
    #return resend_message(chat_id, ev, reply_to=reply_to, force_caption=force_caption)

def send_to_single(ev, msid, user, *, reply_msid=None, force_caption=None, photo=None):
...
def wrap_core(func, reply_to=False, im=None):
    def f(ev):
        m = func(UserContainer(ev.from_user))
        exit_code = 2
        if isinstance(m, list) and isinstance(m[-1], int):
            exit_code = m.pop(-1)
        else:
            pass

        if exit_code != 2 and im: # everything after this is a HACK
            send_to_single_inner_photo(ev.chat.id, ev, reply_to=reply_to, photo=im)
...
def cmd_info(ev, arg):
    c_user = UserContainer(ev.from_user)
    if ev.reply_to_message is None and arg == '':
        return send_answer(ev, core.get_info(c_user), True)
    elif not ev.reply_to_message and arg:
        return send_answer(ev, core.get_info_mod(c_user, None, arg), True)
...
#@takesArgument(optional=True)
#def cmd_iamgod(ev, arg):
	#c_user = UserContainer(ev.from_user)
	#if ev.reply_to_message is None and arg == '':
		#return send_answer(ev, core.setgod(c_user), True)
#
	#reply_msid = ch.lookupMapping(ev.from_user.id, data=ev.reply_to_message.message_id)
	#if reply_msid is None:
		#return send_answer(ev, rp.Reply(rp.types.ERR_NOT_IN_CACHE), True)
...
def cmd_vtripcode(ev, arg):
    ...
    if arg == "":
        ...
        send_answer(ev, core.set_vtripcode(c_user, arg))
...
def cmd_say(ev, arg):
    c_user = UserContainer(ev.from_user)
    arg = escape_html(arg)
    return send_answer(ev, core.send_rank_message(c_user, arg), True)
...
def cmd_listmods(ev):
    c_user = UserContainer(ev.from_user)
    send_answer(ev, core.list_mods(c_user, RANKS.mod), True)

def cmd_listadmins(ev):
    c_user = UserContainer(ev.from_user)
    send_answer(ev, core.list_mods(c_user, RANKS.admin), True)
